...
func main() {
	fmt.Println("Welcome to Advent of Code Day 7")
...
func isThreeOfAKind(cardRepr []int) bool {
	// Assume we have already checked for better hands
	if slices.Max(cardRepr) == 3-getJokerCount(cardRepr) {
		return true
	} else {
		return false
	}
...
func isFourOfAKind(cardRepr []int) bool {
	...
	if slices.Max(cardRepr) == 4-getJokerCount(cardRepr) {
		return true
	} else {
		return false
	}
...
func isFiveOfAKind(cardRepr []int) bool {
	if slices.Max(cardRepr) == 5-getJokerCount(cardRepr) {
		return true
	} else {
		return false
	}
...
func getJokerCount(cardRepr []int) int {
	return cardRepr[1]
}
