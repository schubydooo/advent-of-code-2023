...
func main() {
	...
	expandCurrentNumber := func(loc coordinate) (coordinate, int) {
		...
		fmt.Println("             full number: ", num)

		return startLoc, num
	}

	getAdjacentNumbers := func(loc coordinate) map[coordinate]int {
		adjNumbers := make(map[coordinate]int)
		...
		for _, dir := range directions {
			...
			if inspect_x >= 0 && inspect_y >= 0 && inspect_x < len(schematic[loc.Y]) && inspect_y < len(schematic) {
				...
				if unicode.IsDigit(inspect_val) {
					...
					adjNumbers[startLoc] = num // Add to map
				}
			}
		}
		return adjNumbers
	...
	for y, row := range schematic {
		...
		for x, ch := range row {
			...
			if isGearSymbol(ch) {
				...
				fmt.Println("        Found a gear symbol")
				adjNumbers := getAdjacentNumbers(coordinate{X: x, Y: y})

				// Add to solution if valid gear ratio
				if len(adjNumbers) == 2 {
					fmt.Println("Gear ratio found.  This has exactly 2 numbers")
					keys := []coordinate{}
					for k, _ := range adjNumbers {
						keys = append(keys, k)
					}

					solution += adjNumbers[keys[0]] * adjNumbers[keys[1]]
				}
			}
		}
	}
